...
struct CvForkParams
{
    int numChain;
    int chainLength;
    CvForkParams(int _numChain, int _chainLength);
};
...
public:
    ...
    bool ForkTrain( const std::string _cascadeDirName,
                    std::vector<std::string> &_posFilename,
                    const std::string _negFilename,
                    std::vector< int > &_numPos, std::vector< int > &_numNeg,
                    int _precalcValBufSize, int _precalcIdxBufSize,
...
private:
    ...
    int strongpredict( int sampleIdx, bool isPositive, int currStart, int chainlength );
    ...
    bool updateForkTrainingSet( int currStart, int chainlength );
    int fillForkPassedSamples( int first, int count, bool isPositive, int64& consumed, int currStart, int chainlength );
