...
CvForkParams:: CvForkParams(int _numChain, int _chainLength)
{
    numChain=_numChain;
...
bool CvCascadeClassifier::ForkTrain( const string _cascadeDirName,
                                vector<string> &_posFilename,
                                const string _negFilename,
                                vector< int > &_numPos, vector< int > &_numNeg,
                                int _precalcValBufSize, int _precalcIdxBufSize,
                                const CvForkParams& forkParams,
                                const CvCascadeParams& _cascadeParams,
                                const CvFeatureParams& _featureParams,
                                const CvCascadeBoostParams& strongParams,
                                const CvCascadeBoostParams& chainParams )
{
    int startChain, strongCurr, chainlength=forkParams.chainLength, numChain=forkParams.numChain;
    string dirName;
    if (_cascadeDirName.find_last_of("/\\") == (_cascadeDirName.length() - 1) )
        dirName = _cascadeDirName;
    else
    ...
    numStages = chainlength*numChain;
    ...
    if ( !load( dirName ) )
    {
        ...
        *stageParams = strongParams;
        ...
        cout << "PARAMETERS:" << endl;
        cout << "cascadeDirName: " << _cascadeDirName << endl;
        cout << "vecFileName: " << _posFilename[0] << endl;
        cout << "bgFileName: " << _negFilename << endl;
        cout << "numPos: " << _numPos[0] << endl;
        cout << "numNeg: " << _numNeg[0] << endl;
        cout << "numStages: " << numStages << endl;
        cout << "precalcValBufSize[Mb] : " << _precalcValBufSize << endl;
        cout << "precalcIdxBufSize[Mb] : " << _precalcIdxBufSize << endl;
        cascadeParams.printAttrs();
    ...
    bool paramSwitch=startNumStages<chainlength ? true : false;
    if(!paramSwitch) *stageParams = chainParams;

    const clock_t begin_time = clock();
    bool firstchain=true;
    for(int chain=startNumStages/chainlength; chain<numChain; chain++)
    {
        // Start recording clock ticks for training time output

        if( _cascadeDirName.empty() || _posFilename[chain].empty() || _negFilename.empty() )
            CV_Error( CV_StsBadArg, "_cascadeDirName or _bgfileName or _vecFileName is NULL" );

        numPos = _numPos[chain];
        numNeg = _numNeg[chain];
        featureEvaluator->init( featureParams, numPos + numNeg, cascadeParams.winSize );

        if ( !imgReader.create( _posFilename[chain], _negFilename, _cascadeParams.winSize ) )
        {
            cout << "Image reader can not be created from -vec " << _posFilename[chain]
                    << " and -bg " << _negFilename << "." << endl;
            return false;
        }
        startChain=firstchain ? startNumStages % chainlength : 0;
        firstchain=false;
        for( int i = startChain; i < chainlength; i++ )        //available stages will be relearned!!!
        {
            cout << endl << "===== TRAINING " << chainlength*chain+i << "-stage =====" << endl;
            cout << "<BEGIN" << endl;

            strongCurr=chain==0 ? i : chainlength;

            if ( !updateForkTrainingSet( chainlength*chain, strongCurr ) )
            {
                cout << "Train dataset for temp stage can not be filled. "
                        "Branch training terminated." << endl;
                break;
            ...
            // save current stage
            char buf[10];
            sprintf(buf, "%s%d", "stage", chain*chainlength+i );
            string stageFilename = dirName + buf + ".xml";
            FileStorage fs( stageFilename, FileStorage::WRITE );
            if ( !fs.isOpened() )
            {
    ...
    save( dirName + CC_CASCADE_FILENAME, false );
...
int CvCascadeClassifier::strongpredict( int sampleIdx, bool isPositive, int currStart, int chainlength )
{
    CV_DbgAssert( sampleIdx < numPos + numNeg );
    if(isPositive)
    {
        for (vector< Ptr<CvCascadeBoost> >::iterator it = stageClassifiers.begin()+currStart;
            it < stageClassifiers.end(); it++ )
        {
            if ( (*it)->predict( sampleIdx ) == 0.f )
                return 0;
    ...
    {
        for (vector< Ptr<CvCascadeBoost> >::iterator it = stageClassifiers.begin();
            it != stageClassifiers.begin()+chainlength; it++ )
        {
            if ( (*it)->predict( sampleIdx ) == 0.f )
                return 0;
        }
...
bool CvCascadeClassifier::updateForkTrainingSet( int currStart, int chainlength )
{
    int64 posConsumed = 0, negConsumed = 0;
    imgReader.restart();
    int posCount = fillForkPassedSamples( 0, numPos, true, posConsumed, currStart, chainlength );
    if( !posCount )
        return false;
    cout << "POS count : consumed   " << posCount << " : " << (int)posConsumed << endl;

    int proNumNeg = cvRound( ( ((double)numNeg) * ((double)posCount) ) / numPos ); // apply only a fraction of negative samples. double is required since overflow is possible
    int negCount = fillForkPassedSamples( posCount, proNumNeg, false, negConsumed, currStart, chainlength );
    if ( !negCount )
        return false;
...
int CvCascadeClassifier::fillForkPassedSamples( int first, int count, bool isPositive, int64& consumed, int currStart, int chainlength )
{
    int getcount = 0;
    ...
    for( int i = first; i < first + count; i++ )
    {
        for( ; ; )
        {
            ...
            if( strongpredict( i, isPositive, currStart, chainlength ) == 1.0F )
            {
                getcount++;
