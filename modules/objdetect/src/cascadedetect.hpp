...
protected:
    ...
    template<class FEval>
    friend int predictForkCategorical( CascadeClassifierImpl& cascade, Ptr<FeatureEvaluator> &featureEvaluator, double& weight, int numChain);
    ...
    template<class FEval>
    inline int predictForkCategoricalStump( CascadeClassifierImpl& cascade, Ptr<FeatureEvaluator> &_featureEvaluator, double& sum, int numChain );

    int runAt( Ptr<FeatureEvaluator>& feval, Point pt, int scaleIdx, double& weight, int numChain );
...
template<class FEval>
inline int predictForkCategorical( CascadeClassifierImpl& cascade,
                               Ptr<FeatureEvaluator> &_featureEvaluator, double& sum, int numChain )
{
    int nstages = (int)cascade.data.stages.size();
    int nodeOfs = 0, leafOfs = 0, chainlong=nstages/numChain;
    FEval& featureEvaluator = (FEval&)*_featureEvaluator;
    size_t subsetSize = (cascade.data.ncategories + 31)/32;
    int* cascadeSubsets = &cascade.data.subsets[0];
    float* cascadeLeaves = &cascade.data.leaves[0];
